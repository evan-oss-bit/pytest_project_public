...
print('是否立即执行sql语句：', is_exec_sql)
print('新数据库：', db_new_param)
print('旧数据库：', db_old_param)
print('diff保存位置：', diff_file_path)
...
def print_data(data):
    if hasattr(data, '__iter__') and callable(data.__iter__()):
...
def build_alter_table_sql(fields_new, fields_old):
    ...
    for item in fields_new:
        ...
        if old is None:
            ...
            continue
        # 修改
        if item['Type'] != old['Type'] or operator.eq(item['Default'], old['Default']) != 0 or item['Extra'] != old[
            'Extra'] or \
                item['Null'] != old['Null']:
            print(item)
            print(old)
...
def ex(db, *sqls):
    for sql in sqls:
        try:
            db.query(add(sql, ''), processed=True)
...
def compare_foriegn_key(fk, other_fk):
    for key, value in fk:
        if key == 'TABLE_SCHEMA' or key == 'TABLE_COMMENT':
            continue
        if operator.eq(value, other_fk[key]) != 0:
            return False
    return True
